...
bool containsKMer(kh_S64_t *kMers, int64_t kMer, int k, bool complements) {
...
void eraseKMer(kh_S64_t *kMers, int64_t kMer, int k, bool complements) {
...
int64_t nextKMer(kh_S64_t *kMers, size_t &lastIndex) {
...
std::vector<int64_t> kMersToVec(kh_S64_t *kMers) {
    std::vector<int64_t> res(kh_size(kMers));
    size_t index = 0;
